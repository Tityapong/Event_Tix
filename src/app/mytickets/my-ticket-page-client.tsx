...
// Replace the mock functions with your actual API imports
import { getOrders, scanQRCode, type OrderResponse } from "@/lib/order"

// Dynamically import Html5Qrcode to avoid SSR issues
const Html5QrcodeScanner = dynamic(() => import("./qr-scanner-component"), { ssr: false })
...
export default function MyTicketPageClient() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const tickets: DashboardTicket[] = orders.map((order) => ({
        ...
        image: order.eventImage || "/placeholder.svg?height=200&width=400",
  ...
  const handleQRScanSuccess = useCallback(
    ...
    [selectedTicket, upcomingTickets, pastTickets, fetchOrders],
  )
        ...
        <Tabs defaultValue="upcoming" className="space-y-8">
          ...
          <TabsContent value="past" className="space-y-8">
            ...
            {memoizedPastTickets.length > 0 ? (
              ...
              <Card className="border-0 bg-white/80 backdrop-blur-lg shadow-xl">
                <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                  <h3 className="mb-3 text-2xl font-bol
d text-slate-800">No Past Events</h3>
                  <p className="mb-8 text-slate-600 max-w-md">Looks like you haven not attended any events yet.</p>
                </CardContent>
              </Card>
            )}
          </TabsContent>
            ...
            <div className="space-y-6">
              <div className="overflow-hidden rounded-2xl border-0 shadow-2xl bg-white">
                ...
                {showQR ? (
                  ...
                  <div className="flex flex-col items-center justify-center bg-gradient-to-br from-white to-slate-50 p-8">
                    <Html5QrcodeScanner onScanSuccess={handleQRScanSuccess} onScanError={handleQRScanError} />
                    <p className="text-sm text-slate-600 font-medium mb-4 text-center">
                      Align the QR code within the scan area
                    </p>
